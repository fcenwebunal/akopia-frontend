...
### 2026-08-18 (noche) — Las dos capas no bastaban: `:not()` en vez de una carrera de precedencia

Juan Manuel reportó, con capturas del sitio ya desplegado (Vercel), daño real: una franja de la portada ("Importante: si deseas donar...") colapsada casi al punto de ilegible, las redes sociales y el selector de idioma del cabezote ausentes, y pidió además quitar la miga de pan por completo (no la quería, no es que se hubiera roto) y que el buscador se viera como el de unal.edu.co.

**La separación en dos capas de la entrega anterior no era suficiente — encontrado con evidencia real, no en teoría:**

- **Bootstrap y Tailwind comparten nombres de clase de utilidad exactos** — `.overflow-hidden`, entre otras. Mi heurística anterior ("selector con id o clase de la plantilla → capa alta") clasificaba `.overflow-hidden` de Bootstrap como "calificado" (tiene un punto, una clase), dejándolo sin capa — con lo que le ganaba al `.overflow-hidden` de Tailwind en el mismo nombre, aplicado al mismo `<div>` real de la portada. De ahí la franja colapsada.
- **Las utilidades responsivas de Bootstrap viven dentro de `@media`**, y mi script trataba cualquier regla dentro de un `@media` como "capa alta" en bloque, sin mirar el selector real de cada una. Con `.d-none`/`.d-md-block` usando `!important` (como los define Bootstrap), quedar en capas distintas por accidente **invierte** el orden de precedencia para las reglas importantes — `.d-none` terminaba ganando en cualquier ancho de pantalla, por eso las redes sociales y el idioma nunca se veían.

**La solución que reemplazó por completo el sistema de capas:** en vez de hacer que la plantilla y Tailwind compitan por prioridad (frágil, con casos límite que ya mordieron dos veces), `scripts/scope-unal-template-css.mjs` ahora le agrega a **cada** selector de la plantilla `:not(.akopia-content, .akopia-content *)` — una exclusión positiva, no una carrera. `<UnalShell>` envuelve `{children}` en un `<div className="akopia-content">`; ninguna regla de Bootstrap/unal.css puede aplicar ahí dentro, sin importar nombre de clase, especificidad o `!important`, porque el selector completo simplemente no calza. El cabezote/pie/panel de accesibilidad, que nunca llevan esa clase, quedan sin ningún cambio de comportamiento.

**Un hallazgo aparte, no relacionado con capas:** con las redes sociales y el idioma ya visibles, el escudo (`position:absolute`, 135px de alto) volvió a desbordarse sobre el menú de Sedes — la altura natural de `#unalTop` depende de que sus filas (redes/idioma + subdominio/buscador/sedes) sumen esa misma altura, y con menos contenido propio de AKOPIA que el menú de ejemplo original de la plantilla, no siempre llegan. Resuelto con `min-height: 145px` en línea sobre `#unalTop` — no cambia el diseño cuando hay espacio de sobra, solo evita que colapse cuando no lo hay.

**Miga de pan retirada por completo** (`breadcrumb.tsx` eliminado, ya no se monta desde `<UnalShell>`) — pedido explícito, revierte la pieza de la entrega anterior.

**Buscador:** el widget de Google CSE sí carga y responde (confirmado con las peticiones de red reales, incluida la publicidad de Google que trae el plan gratuito) — no es un placeholder muerto. No llegó a verse pixel-a-pixel idéntico al de unal.edu.co (campo blanco con bordes redondeados); es un matiz de estilo de un widget de terceros, no una funcionalidad rota, y queda pendiente si se quiere perseguir más.

Verificado de nuevo con Playwright, contra el servidor real: la portada completa (las 6 secciones) sin ninguna regresión de diseño, cabezote con redes sociales + idioma + buscador + Sedes en su lugar sin superposición, sesión real en `/panel` y `/panel/inventario` sin miga de pan y sin ningún cambio de estilo en el contenido. `npx tsc --noEmit` y `npm run build` limpios.

### 2026-08-18 (noche) — "Registrarse" invisible, todo el login ilegible en oscuro, y las redes sociales seguían sin verse

Tres hallazgos más en la misma familia de problema (Bootstrap/Tailwind compitiendo por el mismo nombre de clase), reportados por Juan Manuel con capturas: el botón "Registrarse" sin contraste en tema claro, toda la pantalla de login casi ilegible en tema oscuro, y las redes sociales/idioma del cabezote — que en la entrega anterior quedaron *codificadas* pero nunca realmente verificadas en pantalla — seguían sin aparecer.

- **`AccountBar` vivía fuera de la protección.** La exclusión `:not(.akopia-content, .akopia-content *)` de la entrega anterior solo cubre lo que está DENTRO de `.akopia-content` — pero `<AccountBar>` (con sus propios botones Tailwind, "Iniciar sesión"/"Registrarse"/"Salir") vive en `<UnalShell>` como hermano de `<main>`, fuera de esa frontera. Una regla tan simple como `a{...}` de la plantilla (sin capa, siempre gana) le borraba el fondo verde y el texto blanco al link. Resuelto agregándole la clase `akopia-content` a su propio contenedor raíz — no hacía falta reestructurar nada más.
- **El fondo del contenido no seguía el tema.** El refuerzo de la entrega anterior (`text-(--ink) font-sans` en `<main>`) arregla el *texto*, pero `background-color` no es una propiedad heredable — sin pintar un fondo explícito ahí, el `body{background-color:#fff}` de Bootstrap (sin escopear, a propósito, por el panel de accesibilidad) se veía *a través* de todo, fijo en blanco, sin importar el tema. En oscuro: texto claro sobre fondo blanco fijo, casi ilegible. Se agregó `bg-(--surface)` al mismo `<main>`.
- **Las redes sociales estaban ahí, pero fuera de la pantalla — literalmente detrás del escudo.** Al quitar la fila de "Perfiles" (decisión de la entrega anterior), se quitó también el primer hijo del `<nav>` flex que empujaba `.socialLinks`/`.languageMenu` hacia la derecha con el espacio que ocupaba. Sin él, quedaban pegados al borde izquierdo, tapados por el escudo. Arreglado con `.ml-auto` en `.socialLinks` (Bootstrap **4**, confirmado en el propio `bootstrap.min.css` — no es `.ms-auto`, que es de Bootstrap 5 y no existe en este archivo).
- **Otra colisión de nombre exacta, esta vez `.collapse`.** Tailwind también trae una utilidad `.collapse` (`visibility:collapse`, pensada para filas de tabla) que comparte nombre con el `.collapse` de Bootstrap (el mecanismo de colapso de componentes). El `<div>` del selector de idioma la tenía en el `className` original de la plantilla; como nada más en toda la cascada fija `visibility` para ese elemento, la regla de Tailwind aplicaba sin competencia y lo dejaba con `visibility:collapse` (invisible). No hacía falta — el despliegue del idioma ya lo maneja `dropdown-toggle`, no `collapse` — así que se quitó del `className`.
...
### 2026-08-18 (noche) — Hueco gris en el cabezote, y la franja de cuenta se une al panel de accesibilidad

Reportado con captura anotada de `/panel/usuarios`: una franja gris suelta entre el menú y "Panel de Accesibilidad", y pedido explícito de mover "Salir"/"Iniciar sesión" a la izquierda de esa misma pestaña en vez de su propia fila debajo. El tercer punto del pedido (bordes y fondo del área de contenido) resultó ya estar resuelto por el fix de `bg-(--surface)` de la entrega anterior — confirmado con una verificación local fresca antes de tocar nada más ahí.

- **La franja gris era el `min-height:145px` del cabezote sin nada que lo pintara.** El contenido real (redes+idioma, 30px, más subdominio+buscador+menú+Sedes, 73px) solo llega a 103px; el sobrante hasta 145px —para que quepa el escudo, que es `position:absolute` y no empuja la altura por su cuenta— quedaba sin ningún `background-color` propio en `#unalTop`, dejando ver el gris de la página detrás. Resuelto pintando ese sobrante con el mismo tono que ya usa `.firstMenu` (`rgb(102,102,102)`), para que se vea como cabezote continuo en vez de un hueco.
- **`<AccountBar>` se movió de su propia franja a dentro de `<AccessibilityPanel>`**, renderizada antes de `#pestania-accesibilidad` (que trae `float:right` de la propia plantilla) — con `float:left` propio y la misma altura de línea (35px), cae naturalmente a la izquierda en la misma fila, sin tocar el CSS de la plantilla. Su diseño se simplificó de "franja de ancho completo con fondo" a un grupo compacto de enlaces, ya que ahora comparte fila con la pestaña en vez de tener la suya.
- **Efecto de paso:** al desaparecer la franja propia de `<AccountBar>`, `contentTop` (la medición que ya usa `<UnalShell>` para posicionar el indicador de "Servicios", entrega anterior) se recalcula solo — sigue funcionando sin ningún cambio de lógica, porque mide directamente dónde empieza `<main>`, no suma alturas a mano.

Verificado con Playwright, en claro y oscuro, en `/panel/usuarios` (el caso reportado) y en la portada: cabezote sin ningún hueco de color, "Iniciar sesión"/"Registrarse" y "Hola, {nombre} · Salir" en la misma fila que "Panel de Accesibilidad", a su izquierda. `npx tsc --noEmit` y `npm run build` limpios.

### 2026-08-18 (noche) — "Junto a", no "en la esquina opuesta"; y las franjas grises sueltas, unificadas con el blanco del contenido

Malentendido real de la entrega anterior, señalado con una captura anotada: "a la izquierda de X" se leyó como "alineado al borde izquierdo de la fila", cuando Juan Manuel pedía "pegado a X, a su izquierda". Con `<AccountBar>` en `float:left` y `#pestania-accesibilidad` en `float:right` (fijo de la plantilla), cada uno se iba a su extremo — quedaban en esquinas opuestas de una fila de ancho completo, más lejos entre sí que en el diseño original con su propia franja.

- **Corregido invirtiendo el mismo mecanismo:** `<AccountBar>` pasó a `float:right` también, y se reordenó **después** de `#pestania-accesibilidad` en el DOM — con floats del mismo lado, el primero en aparecer llega al borde y el siguiente se acomoda contra él, así que ahora "Hola, {nombre} · Salir" queda pegado a la izquierda de la pestaña, no en la esquina opuesta.
- **Las franjas grises sueltas** (la fila de accesibilidad/cuenta, los márgenes laterales del 10% que trae `.detalle`) eran huecos transparentes de la plantilla mostrando lo que hubiera detrás — nunca se les había pintado nada. Se resolvió pintando el fondo del propio `.unal-chrome` (el contenedor raíz) con `bg-(--surface)`, el mismo blanco/oscuro-según-tema que ya usa el contenido — no afecta al cabezote ni al pie, que ya pintan su propio fondo opaco de punta a punta por encima.
...
### 2026-08-19 — Iconografía recortada al 25%, en el dashboard y en los botones de contenido

Pedido explícito, con la catedral de la portada como referencia visual: agregar íconos a las tarjetas del panel principal y a los botones de las pantallas de operación, con el mismo recurso decorativo — un ícono más grande que su contenedor, empujado hacia el borde derecho y recortado ahí un 25% por el `overflow-hidden` del padre. Dos decisiones resueltas con `AskUserQuestion` antes de tocar código: **`lucide-react`** (ya era dependencia del proyecto desde el 18 de agosto, sin necesidad de instalar nada) en vez de dibujar SVG a mano, y **un componente de botón compartido nuevo** en vez de repetir el tratamiento archivo por archivo — el explorador confirmó que no existía ninguno; cada botón repetía sus propias clases de Tailwind.

- **`src/components/ui/cut-icon.tsx`** — `<CutIcon>`, la pieza reutilizable: un ícono de `lucide-react` en `absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/4`, con `currentColor` (hereda el color de texto del padre, sin token de color aparte). El padre necesita `relative overflow-hidden` — la misma mecánica que ya usaba la catedral de la portada (`(landing)/page.tsx`), solo que ahí es un `<img>` suelto y aquí es un componente reutilizable en vez de repetir la técnica a mano.
- **`src/components/ui/button.tsx`** — `<Button>` (elemento) y `<LinkButton>` (`next/link`), con variantes (`primary`, `outline`, `danger`, `danger-solid`, `ghost`), dos tamaños, `loading` (integra `Spinner`, reemplazando el patrón repetido `{busy ? <Spinner/> : null}` que había en cada pantalla) e `icon` opcional que dibuja el `CutIcon` al 25% de opacidad. Reemplaza los `<button>`/`<Link>` con clases sueltas en las pantallas donde se aplicó.
- **`StatTile`** (`src/components/panel/charts.tsx`) y el componente `Action` de `/panel` ganaron la misma técnica directamente (sin pasar por `<Button>`, porque son tarjetas, no botones): ícono grande al 12% de opacidad, coloreado según el `tone` de la tarjeta (verde/amarillo/rojo/gris).
- **Alcance real, no exhaustivo:** se aplicó a las tarjetas del panel (4 de estado + 4 de "Acciones"), a los botones principales de cada listado (Registrar donación/solicitud/despacho, Productos faltantes, Vincular correo, Crear/Descargar respaldo), a los botones de decisión de las pantallas de detalle (Aprobar/Rechazar/Cancelar en solicitudes, Marcar apto/A revisión/Rechazar en donaciones, Liberar/Rechazar/Reubicar en inventario) y a los envíos finales de los tres formularios "nueva". **No** se tocó el menú superior de la plantilla institucional (`menu-config.ts` — es cromo de Unimedios, no un botón de contenido de AKOPIA) ni los botones menores de utilidad (Copiar, Actualizar, cerrar diálogo) — quedan con su estilo de texto/ícono simple de siempre.
- Efecto de paso, limpieza: cada archivo tocado perdió su patrón manual `{busy ? <Spinner/> : "Texto"}` en favor de la prop `loading` de `<Button>` — menos código repetido, mismo comportamiento visible.

**No verificado visualmente en navegador real** — este entorno no tiene Playwright ni otra herramienta de automatización de navegador disponible en la sesión. Verificado en su lugar: `npx tsc --noEmit` limpio, `npm run build` limpio (compila y prerenderiza las 26 rutas sin error), y un arranque real de `npm run dev` con `curl` confirmando que las páginas responden 200 sin excepción de servidor. Pendiente que Juan Manuel confirme el resultado visual en su navegador, en claro y oscuro.

### 2026-08-19 — Módulo de dirección con mapa: nomenclatura colombiana, Mapbox, react-hook-form + zod

Pedido explícito: un componente que combine tipo de vía/número/placa/complemento (dropdowns, react-hook-form + zod), autocompletado restringido a Colombia/Manizales, y un mapa con pin arrastrable que haga geocoding inverso al soltarlo. Dos decisiones reales, resueltas con `AskUserQuestion` antes de escribir código porque cambiaban el diseño y el costo:

1. **Proveedor de mapa: Mapbox, no Google Maps.** El mapa que ya existía (Leaflet + teselas CARTO, en `map-picker.tsx`/`help-map.tsx`) no pide llave ni tarjeta — es justo lo que permitió esquivar la tarjeta rechazada de Fly.io el 18 de agosto. Google Maps exige tarjeta desde el arranque; Mapbox tiene una capa gratuita generosa sin pedirla al principio. Se descartó también Nominatim/OSM (gratis, sin llave, mismo esquema que CARTO) porque el pedido explícito era autocompletado y geocoding inverso reales para nomenclatura colombiana, donde Mapbox es más preciso.
2. **Sí se migra el backend** para guardar street_type/street_number/street_plate/address_complement estructurados, no solo componer el texto en la interfaz — evita el mismo problema que ya tiene el catálogo sin lotes: si algún día hace falta la descomposición, con datos ya cargados sería irrecuperable. Migración `043` en akopia-backend, detalle en su propio `CLAUDE.md`.

**`src/lib/address.ts`** — `STREET_TYPES` (los diez valores del esquema), `composeAddress()` (arma el texto que se guarda en `destination` y se manda a geocodificar) y `parseAddress()`, que intenta descomponer un texto libre en tipo de vía/número/placa por expresión regular. **Esto es best-effort, no una garantía:** ningún geocodificador (Mapbox incluido) devuelve esos cuatro campos por separado para una dirección colombiana — parsear su resultado de vuelta a la nomenclatura es una aproximación que siempre queda como valor de partida editable en el formulario, nunca como el dato final sin que el operador lo confirme.

**`src/lib/mapbox.ts`** — `forwardGeocode()`/`reverseGeocode()` contra la API REST de Mapbox directo con `fetch`, sin el SDK oficial (`@mapbox/mapbox-sdk`), que sería una dependencia de más para dos llamadas. Autocompletado sesgado a Manizales (`proximity` + `bbox`) pero sin restringir a la ciudad exacta — una donación puede venir de un municipio vecino. `MAPBOX_ENABLED` es `false` si `NEXT_PUBLIC_MAPBOX_TOKEN` no está puesta: como Claude no puede crear la cuenta de Mapbox de Juan Manuel (mismo motivo que Fly.io/Vercel/Railway el 18 de agosto), el código queda listo pero inerte hasta que él la cree y pegue el token — sin esa llave, `AddressMapField` sigue mostrando los cuatro campos y compone el texto igual, solo sin buscador, mapa ni geocoding automático, con un aviso explícito en vez de fallar en silencio o tronar.

**`src/components/app/mapbox-picker.tsx`** — el mapa en sí, mismo patrón que ya usaba `map-picker.tsx` con Leaflet (pin propio en vez del ícono de fábrica, que rompe con el empaquetado de Next.js; siempre cargado con `next/dynamic(..., { ssr: false })` porque `mapbox-gl` toca `window` al importarse). Sincroniza el pin cuando `lat`/`lng` cambian desde afuera (escribir una dirección, elegir una sugerencia) sin recrear el mapa completo — solo mueve el marcador si la diferencia es real, para no pelear con el propio arrastre del operador.

**`src/components/app/address-map-field.tsx`** — `<AddressMapField>`, la pieza que junta las tres vías (dropdowns, buscador, mapa) en un solo `AddressValue` hacia el padre. El riesgo de diseño real, no anticipado hasta programarlo: sin cuidado, arrastrar el pin dispara un geocoding inverso que resetea los cuatro campos, lo que a su vez dispara el geocoding *directo* de esos campos, que puede devolver una coordenada ligeramente distinta de donde el operador soltó el pin — un "salto" del punto justo después de soltarlo. Resuelto con una bandera (`skipNextCompose`) que se activa antes de cualquier `reset()` propio (sincronizar desde el padre, elegir una sugerencia, o el resultado del geocoding inverso) para que el efecto de composición/geocoding no reaccione a ese cambio como si el operador hubiera tecleado los campos a mano.

**Segundo cuidado real:** con los cuatro campos vacíos, `composeAddress()` devuelve texto vacío — sin control, esto habría borrado el destino de una solicitud o despacho ya existente (creados antes de esta migración, con `destination` en texto libre y ningún campo estructurado) apenas se abriera su formulario de edición. Se resolvió conservando el texto anterior cuando la composición estructurada no produce nada, en vez de sobrescribirlo con vacío.

**Integrado en las tres pantallas reales con formulario de dirección + mapa** (auditado con `grep` antes de tocar nada — `solicitudes/[id]` solo *muestra* coordenadas con `CoordinatesDisplay`, no las edita, así que queda fuera): `solicitudes/nueva` (destino de la solicitud), `despachos/nueva` (hereda el punto de la solicitud elegida, incluidos los cuatro campos estructurados si ya los tiene) y `despachos/[id]` → "Editar ubicación" (que antes solo dejaba mover el pin; ahora edita también el texto y los campos estructurados a la vez, mismo componente). El mapa de solo lectura del panel de inicio (`help-map.tsx`) y el de un solo punto sin dirección (`map-picker.tsx`, ya sin usar tras esta migración salvo que algo nuevo lo necesite) se quedan en Leaflet/CARTO — no tienen dirección que capturar, no había razón para tocarlos.

**Nuevas dependencias**, con acuerdo explícito antes de instalarlas (la regla del stack — README §2 — pide discutir cualquiera): `react-hook-form`, `zod`, `@hookform/resolvers`, `mapbox-gl` (+ `@types/mapbox-gl`). Documentadas en el README con su propio criterio de cuándo usarlas — no se migra el resto de la app a react-hook-form por consistencia, solo donde el formulario tiene reglas propias que valen la pena tipar.

Verificado: `npx tsc --noEmit` y `npm run build` limpios (26 rutas), `npx eslint` sobre los cuatro archivos nuevos sin errores (una sola advertencia informativa, esperada: React Compiler no memoiza automáticamente un componente que usa `watch()` de react-hook-form — no es un bug, es el trade-off documentado de esa API). Migración `043` confirmada aplicada contra `pb_data` real, detalle en el `CLAUDE.md` del backend. **No probado con un token de Mapbox real** — sin cuenta creada todavía, el buscador/mapa/geocoding quedan sin ejercitar de punta a punta; los cuatro campos estructurados y su composición a texto sí se verificaron (`composeAddress`/`parseAddress` no dependen de Mapbox). Pendiente que Juan Manuel cree la cuenta y pegue `NEXT_PUBLIC_MAPBOX_TOKEN` para la prueba real.

### 2026-08-19 (tarde) — Mapbox pedía tarjeta: se cambia a Nominatim/OSM antes de haberlo desplegado

Juan Manuel reportó que Mapbox le pedía tarjeta para crear la cuenta — el mismo tropiezo que ya sacó a Fly.io del despliegue provisional el 18 de agosto. Investigado antes de decidir: en general Mapbox no debería pedirla para el nivel gratuito (100k geocodificaciones/mes sin tarjeta, según su propia documentación), pero evidentemente a él sí se la pidió — no había forma de saber si era un requisito real de su cuenta/región o solo un paso opcional, así que se resolvió con `AskUserQuestion` en vez de asumir. Decisión: **cambiar a Nominatim/OpenStreetMap**, gratis y sin cuenta — el mismo esquema que ya usan los mapas de la app (CARTO) desde el principio.

**Alcance del cambio, menor de lo que parece:** `AddressMapField` (el componente entero, con react-hook-form + zod, el buscador, la lógica de `skipNextCompose` y `composeAddress`/`parseAddress`) no cambia nada — el contrato `forwardGeocode`/`reverseGeocode`/`GeocodeSuggestion` se mantuvo idéntico, solo cambió quién lo implementa detrás. Dos archivos fuera, cero nuevos:

- **`src/lib/mapbox.ts` → `src/lib/geocoding.ts`** — mismas dos funciones, ahora contra `nominatim.openstreetmap.org` con `fetch` directo (sin SDK, igual que antes). Sin llave que gestionar, así que desaparece el concepto de "`_ENABLED`": el buscador y el geocoding automático ya no tienen un estado "desactivado por falta de configuración", siempre están disponibles.
- **`src/components/app/mapbox-picker.tsx` eliminado** — ya no hacía falta un mapa aparte para Mapbox GL: `AddressMapField` vuelve a usar el `MapPicker` de Leaflet que ya existía (`map-picker.tsx`), el mismo que usan `solicitudes/nueva`/`despachos/nueva` desde antes de esta migración para el caso sin dirección. Un solo componente de mapa en todo el proyecto otra vez, no dos.
- **`mapbox-gl` y `@types/mapbox-gl` desinstalados.** `react-hook-form`/`zod`/`@hookform/resolvers` se quedan — no tienen relación con el proveedor de mapa.
- `NEXT_PUBLIC_MAPBOX_TOKEN` retirada de `.env.example`, `.env.local` y `DEPLOY-PROVISIONAL.md` — no hace falta ninguna variable de entorno nueva para este módulo, a diferencia de lo que se había documentado unas horas antes.

**Verificado contra el servicio real de Nominatim, no simulado** (accesible sin llave, así que sí se pudo probar de punta a punta esta vez, a diferencia del intento con Mapbox): una búsqueda de "Carrera 23, Manizales" y un geocoding inverso del centro de Manizales devuelven resultados reales y en español, con `address.road` ya viniendo limpio ("Carrera 23", "Avenida Gilberto Alzate Avendaño") — justo el campo que `reverseGeocode()` prefiere sobre el `display_name` completo. `parseAddress()` sobre esos mismos nombres reales de vía se comporta como se esperaba. `npx tsc --noEmit`, `npm run build` (26 rutas) y `npx eslint` sobre los archivos tocados, limpios.

**Política de uso de Nominatim** (máximo 1 peticion/segundo, identificar la aplicación con User-Agent o Referer): un `fetch` de navegador no puede fijar un User-Agent propio —es un header prohibido para JavaScript—, pero si manda el Referer real de la página, que es la vía que la propia política contempla para aplicaciones de navegador. El ritmo de tecleo con pausa que ya tenía el componente (400-900ms) se queda por debajo de 1/s para un operador. Documentado en el propio `src/lib/geocoding.ts` para que quede claro por qué no hay ningún header agregado a mano.

### 2026-08-20 — El frontend se adapta a roles múltiples

Consecuencia directa de la migración `044`/`045` del backend (`CLAUDE.md` del backend tiene el detalle completo): `users.role` pasa de string único a arreglo, y aparecen cuatro roles operativos nuevos junto a Administrador y Coordinación.

- **`UserRole`** (`src/lib/pb.ts`) pasa de `"admin" | "operator"` a los seis valores reales, y `AkopiaUser.role` de `UserRole` a `UserRole[]`.
- **`src/lib/roles.ts`** (nuevo) — espejo en el frontend de `utils/roles.js` del backend: `ROLE_LEVELS`, `ROLE_LABELS`, `hasAnyRole()`, `assignableRoles()`. Existe solo para que la interfaz no ofrezca un botón que el servidor de todas formas va a rechazar — la autoridad real sigue siendo el backend, verificado ahí, no aquí.
- **`menu-config.ts`** — `adminOnly: boolean` en cada ítem se reemplaza por `roles?: UserRole[]` en cada hijo, y el grupo "Administración" se oculta por completo si ninguno de sus hijos queda visible (antes era todo-o-nada por `role === "admin"`). Usuarios e Historial ahora los ve también Coordinación (Historial también Comunicaciones); Respaldos se queda exclusivo de Administrador.
- **`/panel/usuarios`** reescrita para selección múltiple: cada fila muestra una casilla por rol que el actor puede asignar (`assignableRoles()`), más los roles que la cuenta ya tenga aunque el actor no pudiera asignarlos de cero (para poder quitarlos). Una cuenta con rol Administrador queda visible pero sin controles editables para un actor que no sea Administrador — mismo criterio que ya aplica el backend (`role:each != "admin"` sobre el registro objetivo).
- **Botones "Registrar donación/solicitud/despacho" y "Confirmar entrega"**, antes visibles para cualquier sesión, ahora se ocultan si el rol de quien mira no está en el conjunto que el backend de todas formas va a aceptar (evita que alguien llene un formulario completo para toparse con un rechazo al final) — donaciones: admin/voluntariado; solicitudes: admin/coordinación/salida; despachos y confirmar entrega: admin/transporte y distribución/salida.
- **`requireActiveUser` de `pb-server.ts` se reemplaza por `requireRole(token, allowed)`**, genérico — la ruta de firma de Cloudinary (`/api/uploads/sign`) ahora exige un rol de los que tocan inventario para `categories`/`products`/`locations`, en vez de "cualquier activo": con el nuevo esquema, Comunicaciones ya no puede editar catálogo, y antes esta ruta le habría dado una firma válida que solo fallaría después, al guardar en PocketBase.
- **`/api/auth/firebase/route.ts`** — el rol por defecto de una cuenta que se autorregistra pasa de `"operator"` a `["voluntariado"]` (arreglo, y un valor de partida razonable) — sin efecto práctico mientras la cuenta siga `active: false`, a la espera de que un admin/coordinación la revise y le asigne su rol real.
...
### 2026-08-20 (noche) — Recepción rápida y clasificación diferida de remesas

Consecuencia directa de la migración `049` del backend (`CLAUDE.md` del backend tiene el detalle completo, incluida la corrección de modelo sobre el estado de cabecera — dos valores, `recepcion`/`clasificada`, nunca un tercero "en cuarentena").

- **`donaciones/nueva`** gana el interruptor "Solo recepción rápida — clasificar los artículos después". Activado: pide peso total (obligatorio en la pantalla, no en el esquema — mismo criterio ya documentado en el backend) y transportista, oculta el explorador de productos, guarda con `status: "recepcion"` y cero artículos. Desactivado: el formulario de siempre, sin ningún cambio de comportamiento — pero ahora **decide el estado inicial según lo que se acaba de capturar** (si algún renglón quedó "sin clasificar", arranca en `recepcion`; si todo lo capturado ya tiene un estado final, arranca `clasificada` de una vez, sin exigir un clic aparte por algo que ya se resolvió en la misma pantalla).
- **`donaciones/[id]`** gana la pieza que de verdad faltaba: "Agregar artículo", que reutiliza el mismo patrón de selector (`ProductPicker` + cantidad/vencimiento/lote/estado) que ya tenía `donaciones/nueva` — no se extrajo a un componente compartido porque los dos casos son genuinamente distintos: `nueva` arma un borrador local que se envía completo al final, `[id]` guarda cada artículo al instante, apenas se confirma, porque encaja mejor con cómo se abre físicamente una remesa (una caja, un artículo, se decide, se sigue). Si se agrega algo a una remesa ya "clasificada", vuelve sola a "recepción" — no puede quedar marcada como terminada con un artículo recién llegado sin resolver.
- **"Cerrar clasificación"** — bloqueado mientras queden artículos `pending`; el peso clasificado arranca precargado igual al declarado (decisión de Juan Manuel: casi nunca hay tiempo de volver a pesar, y lo descartado suele ser insignificante), editable si de verdad hace falta. Con cero artículos pide una casilla de confirmación explícita, para que cerrar una remesa vacía no sea un clic accidental. "Reabrir clasificación" deshace el cierre.
...
### 2026-08-20 (noche) — Bug real: el login fallaba desde cualquier página que no fuera la portada

Juan Manuel reportó no poder entrar desde `https://acopio.manizales.unal.edu.co/login` — "correo o contraseña incorrectos" con una contraseña que sabía correcta, y sí podía entrar sin problema a `/_/` de PocketBase directamente. La consola del navegador tenía la pista exacta: `POST .../login/api/collections/users/auth-with-password 404` — la petición salió con `/login` metido en medio de la ruta de la API.

**Causa real, encontrada leyendo el propio SDK de PocketBase** (`node_modules/pocketbase/dist/pocketbase.es.mjs`, método `buildURL`): cuando `baseURL` no empieza por `"/"`, el SDK arma la URL pegando `location.origin` + `location.pathname` + `baseURL` — pensado para el caso de una app montada bajo una ruta propia (`baseURL: "/subruta"`). El problema: un string **vacío** `""` tampoco empieza por `"/"`, así que cae en la misma rama que una ruta real — el SDK no distingue "vacío, usa el origen tal cual" de "ruta relativa, súmale el origen". Con `NEXT_PUBLIC_PB_URL=""` (el valor que se dejó a propósito el 19 de agosto, ver `pb.ts`) esto funcionaba por pura coincidencia desde la portada (`pathname: "/"`, que no cambia nada al concatenarse) y se rompía en cualquier otra ruta — nadie había hecho un login real desde `/login` en producción hasta que Juan Manuel lo probó hoy, siempre se entraba navegando desde la portada con sesión ya activa o probando por API.
...
### 2026-08-21 — Mensajes de fallo de autenticación, mucho más específicos, más un hallazgo de fondo sobre Firebase

Pedido explícito de Juan Manuel, mientras probaba la app antes del arranque de las 7:00: las alertas de error de `/login` y `/registro` eran genéricas ("Correo o contraseña incorrectos", "Ese correo ya tiene una cuenta") y no orientaban a nadie sobre qué hacer — grave en un sistema que se ve casi igual al SSO real de la UNAL pero es una base completamente aparte.

**Primer hallazgo, antes de escribir nada:** "usuario y contraseña" en AKOPIA no es PocketBase nativo — es el proveedor de **correo/contraseña de Firebase Authentication** (`createUserWithEmailAndPassword`/`signInWithEmailAndPassword`, en `lib/firebase.ts`). PocketBase nativo solo sigue existiendo como respaldo en el login, para cuentas de antes de Firebase (`admin@akopia.org`). Esto cambió todo el diseño: el problema no era "un error de PocketBase mal traducido", era que Firebase trata Google y contraseña como **proveedores separados bajo el mismo correo**, y de ahí salían exactamente los síntomas que describió Juan Manuel.

**Segundo hallazgo, que descartó el plan A:** para dar mensajes específicos hacía falta saber, para un correo dado, qué proveedor(es) tiene ya en Firebase — la función hecha para eso, `fetchSignInMethodsForEmail` (y su equivalente REST, `accounts:createAuthUri`), **no funciona en este proyecto**. Verificado empíricamente contra el Firebase real de AKOPIA (creando y borrando una cuenta de prueba desechable, no contra datos reales): la respuesta es idéntica exista o no la cuenta — protección anti-enumeración de Google, activada por defecto en proyectos nuevos. La alternativa obvia (Admin SDK de Firebase, `getUserByEmail`) exige una cuenta de servicio nueva — un secreto que este proyecto evita a propósito (`firebase-server.ts` ya lo explica: verifica tokens solo con las llaves públicas, sin credenciales de servidor). No se iba a romper esa decisión bajo presión de horario para agregar un secreto nuevo.

**La solución real no necesitó ningún secreto nuevo:** como *todo* inicio de sesión de verdad (Google o contraseña) termina llamando a `/api/auth/firebase`, `users.firebase_uid` en PocketBase ya es una señal fiel de "esta persona completó un inicio de sesión con Firebase alguna vez" — sin tocar Firebase para nada. Nuevo endpoint público `POST /api/auth/email-status` (usa el mismo superusuario de servicio que el puente, extraído a `lib/pocketbase-service.ts` para no duplicarlo) que responde `{exists, linked, fullName}` — `fullName` solo viaja cuando hace falta mostrarlo (el caso de abajo), para no ampliar más de lo necesario lo que revela un endpoint público.

**Los cinco mensajes pedidos, uno por uno:**

1. **Nombre heredado, no editable.** Si un admin ya vinculó el correo (existe en PocketBase, `firebase_uid` vacío) y la persona se registra con contraseña, el campo Nombre pasa a solo lectura, precargado con el nombre real — antes se podía escribir cualquier cosa ahí, que el puente de todas formas ignoraba en silencio sin explicar por qué. Se comprueba al perder el foco el campo Correo.
2. **Ya entró con Firebase antes, intenta con contraseña.** En login, si la cuenta existe y `linked` es verdadero pero la contraseña falló, el mensaje ahora dice explícitamente que use el botón de Google de arriba o vaya a Registro a agregar una contraseña — en vez de un "incorrectos" a secas. En registro, el intento de crear con Firebase falla con `auth/email-already-in-use` (Firebase, no PocketBase); en vez de un callejón sin salida, se ofrece "Confirmar con Google" ahí mismo, que adjunta la contraseña recién escrita a la cuenta de Google ya existente vía `linkWithCredential` — la persona termina con las dos formas de entrar, en un solo flujo.
3. **Contraseña nativa + Google, las dos deben servir.** Este era el hallazgo más serio: alguien que se registró con contraseña e intenta luego "Continuar con Google" con el mismo correo choca con `auth/account-exists-with-different-credential` — un error real de Firebase para el que el mapa de mensajes **no tenía traducción**, cayendo en el "Ocurrió un error inesperado" genérico. Corregido con el patrón estándar de vinculación de Firebase: se captura el error, se pide confirmar la contraseña ya existente, y con esa sesión se adjunta la credencial de Google que venía pendiente dentro del propio error (`GoogleAuthProvider.credentialFromError`). Mismo mecanismo en `/login` y en `/registro`.
4. **Correo que no existe en ningún lado.** `!exists` en la comprobación → mensaje explícito de que AKOPIA es un sistema aparte del usuario/contraseña de la UNAL, con la ruta a Registro.
5. **Correo nuevo + Google.** Sin cambios — ya funcionaba bien.

**Lo que NO se pudo verificar en esta sesión:** los tres estados de `/api/auth/email-status` sí se probaron contra datos reales del PocketBase local (una cuenta vinculada-nunca-usada, una ya vinculada a Firebase, y una inexistente) y respondieron correctamente. Los dos flujos de vinculación con clic real de Google (`account-exists-with-different-credential` → confirmar contraseña → vincular; y Google → confirmar contraseña → vincular) dependen de un popup interactivo real que no se puede simular por script — quedan implementados según el patrón documentado de Firebase, pero sin un clic real de por medio todavía. Vale la pena que alguien los pruebe a mano antes de darlos por completamente cerrados.

`npx tsc --noEmit`, `eslint` y `npm run build` limpios. Desplegado en el servidor de la UNAL la misma noche.

### 2026-08-21 — Decimales reales en peso/cantidad, atributos de talla y unidades por paquete, e identificación del donante

Pedido de Juan Manuel al empezar a probar el Excel que el equipo ya está usando para imputar datos a mano. Detalle completo del modelo (las tres migraciones nuevas, `051`-`053`) en el `CLAUDE.md` del backend; aquí lo propio del frontend.

**El bug real de los decimales no estaba donde parecía.** Los campos de peso/cantidad ya usaban `type="number" step="any" inputMode="decimal"` y el backend ya aceptaba decimales en todos los `NumberField` relevantes desde el principio — nada se veía roto leyendo el código. La causa: en Windows con configuración regional en español, la tecla decimal del teclado numérico escribe **coma**, y un `<input type="number">` en modo período la rechaza en silencio — el valor queda vacío/en 0 sin ningún error visible, así que parece que "la app no deja poner decimales" cuando en realidad nunca se pudo escribir el número completo.

**`src/components/ui/decimal-input.tsx`** (nuevo) — `<DecimalInput>` reemplaza esos `type="number"` por `type="text"` + `inputMode="decimal"`, acepta coma o punto y los normaliza a número antes de propagar `onChange`. **Un detalle real, no trivial:** un input de texto controlado por un `value: number` no puede simplemente formatear `String(value)` en cada render — tipear "12." se convertiría de inmediato a "12" (pierde el punto) y nunca se podría seguir escribiendo el decimal. Se resolvió con un buffer de texto propio que solo se resincroniza con `value` cuando ese valor cambia *desde afuera* (los botones +/-), nunca en cada tecla. La primera versión hacía esa resincronización en un `useEffect` — `eslint-plugin-react-hooks` la rechazó con `react-hooks/set-state-in-effect` (la misma regla que el propio `CLAUDE.md` ya documentaba para otro caso, el 17 de agosto). Corregido con el patrón que react.dev recomienda para "ajustar estado cuando cambia una prop": comparar contra el valor anterior y llamar `setState` directamente en el cuerpo del componente, no en un efecto — sin el renderizado en cascada extra que un efecto habría provocado.

Aplicado en los cuatro campos reales: peso total y cantidad de cada producto en `donaciones/nueva`, peso clasificado y cantidad al agregar un artículo en `donaciones/[id]`.

**`src/components/app/donation-item-attributes.tsx`** (nuevo) — `<SizeField>` y `<PackageCountField>`, más `isGarmentUnit()`/`isPackagedUnit()` (miran el `code` de la unidad del producto, no un atributo nuevo de `products`: como `donation_items.unit_id` siempre coincide con `products.default_unit_id` — invariante ya exigido por el backend desde el 17 de agosto —, basta con mirar la unidad elegida). Se activan condicionalmente en `LineEditor` (`donaciones/nueva`) e `ItemEditor` (`donaciones/[id]`), mismo patrón que ya usan `requires_expiry`/`requires_batch`.

`<SizeField>` es un solo desplegable (Única/XS/S/M/L/XL/XXL + "Numérica…", que abre un campo de texto) — decisión resuelta con `AskUserQuestion` antes de tocar código, junto con las otras tres de este pedido: **talla y unidades por paquete se capturan por remesa, no por producto** (el mismo producto puede llegar con tallas o tamaños de empaque distintos según la donación — igual que el vencimiento o el lote), **acotado solo a donaciones** (Solicitudes/Kits siguen pidiendo por producto y cantidad; el inventario sigue siendo por producto, no por variante), y **las unidades retiradas se desactivan, no se borran** (11 productos reales del catálogo ya usaban Par/Rollo/Saco/Tubo — desactivar los deja funcionando igual, solo dejan de ofrecerse para productos nuevos).

**`catalog-add-form.tsx`** — el selector de unidad al crear un producto/categoría ahora filtra `unit.active !== false` y fija "Unidad" (antes "Pieza", renombrada en la migración `051`) primera, antes del orden alfabético del resto. `catalog.ts` gana `Unit.active?: boolean` — `loadCatalog()` sigue trayendo **todas** las unidades sin filtrar (para que `unitLabel()` siga resolviendo el nombre de una unidad retirada en un producto que ya la usa); el filtro por `active` vive solo en este selector.

**Identificación del donante** — `donaciones/nueva` gana tres campos opcionales: correo (`donor_email` ya existía en el backend desde el 17 de agosto, nunca se había conectado en el frontend), tipo de documento y número (`donor_id_type`/`donor_id_number`, migración `053` — reemplaza el `donor_rfc` mexicano que nunca llegó a usarse). `donaciones/[id]` los muestra en el encabezado cuando existen, y también ahora muestra talla/unidades-por-paquete de cada artículo, junto al vencimiento/lote.

Verificado con `npx tsc --noEmit`, `eslint` (limpio tras corregir el `react-hooks/set-state-in-effect` de `DecimalInput`) y `npm run build` (29 rutas). El modelo de datos se verificó de punta a punta contra una instancia de PocketBase desechable — detalle en el `CLAUDE.md` del backend. **Sin Playwright en esta sesión** — no se probó el tecleo de una coma real en un navegador; la corrección se apoya en el mecanismo documentado del bug (input nativo rechaza el separador decimal según la configuración regional) y en que `DecimalInput` nunca delega el parseo al navegador.

### 2026-08-21 (noche) — Editar y eliminar con motivo obligatorio, en las pantallas reales

Consecuencia directa de la migración `054` y `11_edit_delete_with_reason.pb.js` del backend (`CLAUDE.md` del backend tiene el detalle completo del diseño y las tres decisiones resueltas con `AskUserQuestion`). Aquí, la parte que de verdad hacía falta: hasta hoy no existía **ninguna** pantalla para editar los datos de cabecera de una donación/solicitud/despacho ya creados (solo cambios de estado vía rutas propias) ni para editar un producto del catálogo (solo su foto) — así que esto no era retocar un formulario existente, era construirlo desde cero, con el motivo obligatorio incluido desde el diseño.

**`src/components/app/record-actions.tsx`** (nuevo) — `<EditRecordButton>`/`<DeleteRecordButton>`, genéricos y config-driven por una lista de `EditableField` (texto/textarea/número/select/checkbox) que cada pantalla declara una vez. Un solo componente reutilizado en las seis colecciones, en vez de un formulario por cada una, porque el backend ya es config-driven de la misma forma (`EDITABLE_RECORDS`) — la interfaz solo necesita describir qué campos mostrar y con qué etiqueta; la lista blanca real de qué se puede tocar la sigue imponiendo el backend, no este componente.

- `<EditRecordButton>` abre un diálogo con los campos + un motivo obligatorio (mínimo 5 caracteres, con el placeholder "Ej.: se cometió un error al ingresar el producto" — el ejemplo que dio Juan Manuel), calcula el *diff* real (solo manda los campos que de verdad cambiaron) y llama a la ruta nueva.
- `<DeleteRecordButton>` primero llama a `GET .../delete-check` — si está bloqueado, muestra el motivo del backend en un diálogo simple ("Entendido", sin pedir nada más); si no, recién ahí abre el diálogo que pide el motivo (placeholder "Ej.: producto de prueba"). Adapta su propio texto ("Eliminar" vs. "Desactivar") al `mode` que devuelve el backend, sin que la pantalla que lo usa tenga que saberlo de antemano.

**Un hallazgo real de la API de PocketBase, encontrado verificando contra el backend real, no anticipado:** `changes` viaja como texto JSON (`JSON.stringify()`), no como objeto — un campo objeto en el `DynamicModel` del backend no se vincula a un objeto plano de JS (detalle completo en el `CLAUDE.md` del backend). `src/lib/record-actions.ts` hace ese `JSON.stringify()` una sola vez, para que ninguna pantalla tenga que acordarse de este detalle.

**Wireado en las pantallas reales**, solo visible para admin/coordinación (`hasAnyRole(operator?.role, ["admin", "coordinacion"])` — más estricto que el resto de cada pantalla, que a veces también deja pasar al dueño del registro):

- `donaciones/[id]` — editar/eliminar la donación completa (redirige a la lista al borrar), y editar/eliminar cada artículo **solo cuando no está bloqueado** (`!locked`, la misma variable que ya gobernaba el aviso "Ya afectó inventario" existente — se reutilizó en vez de inventar una condición nueva).
- `solicitudes/[id]` — igual para la solicitud completa, y por renglón solo cuando `status === "pendiente"` (antes de que exista cualquier reserva).
- `despachos/[id]` — editar/eliminar el despacho (el botón de eliminar desaparece del todo, no solo se deshabilita, en cuanto existe una entrega — no tiene sentido ofrecerlo si siempre va a estar bloqueado); la entrega confirmada gana edición (nombre/teléfono/documento/notas) pero nunca un botón de eliminar, con una nota explicando por qué.
- **`/panel/catalogo`** (página nueva, menú → Inventario, solo admin/coordinación) — no existía ninguna pantalla de gestión de catálogo más allá del explorador de solo navegación (`ProductPicker`) y la cámara de foto; se construyó una lista con pestañas (Grupos/Categorías/Productos/Ubicaciones), buscador, y editar/desactivar por fila. A diferencia del resto de la app, **sí** muestra lo desactivado (con una insignia), porque sin eso no habría forma de ver qué se dio de baja y por qué — el resto del catálogo (`ProductPicker`, `loadCatalog()`) lo sigue filtrando, sin cambios.
- `kits/[id]` — se agrega "Eliminar kit" (en realidad desactiva, mismo criterio) junto a "Guardar como nuevo kit". **No** se tocó la edición de nombre/descripción del kit, que ya tenía su propio mecanismo liviano sin motivo obligatorio desde el 20 de agosto — deliberado: los kits se diseñaron a propósito sin la capa "solo admin/coordinación edita" que sí tiene el resto del catálogo, y forzar el motivo ahí habría sido una regresión de una decisión de producto tomada el día anterior, no algo pedido hoy. Tampoco se tocó `kit_items` (ya se podían quitar libremente, sin motivo, por diseño explícito — "mantenimiento normal de una plantilla viva").
- **`/panel/inventario`** — `ProductLocationDetail` gana "Ajustar cantidad disponible" (solo admin/coordinación), que llama directo a `pb.collection("adjustments").create()` — a diferencia de las demás, esta va por el flujo YA EXISTENTE del backend desde el 17 de agosto (`03_inventory.pb.js` ya tiene un hook que genera el movimiento correcto), pero **nunca había tenido una pantalla real que lo usara**. Era la pieza prometida al resolver la pregunta de "qué hacer cuando algo ya afectó inventario" — el mensaje de bloqueo de la ruta nueva dice "usa un ajuste de inventario", y ahora ese ajuste es una acción real, no una referencia a algo que no existía.
- **`/panel/historial`** — `ENTITY_LABELS` suma `products/categories/groups/locations/kits/kit_items/adjustments`, y cada entrada ahora muestra el motivo (`entry.notes`) cuando existe — el campo ya estaba en el esquema y llegaba en cada petición, pero nunca se pintaba. Es la pieza que de verdad cierra "que quede en el historial": antes de hoy, ninguna acción en toda la app escribía nunca ese campo.
...
### 2026-08-22 (noche) — Panel principal: "Acciones" arriba, y seis dashboards nuevos

Pedido explícito de Juan Manuel: subir "Acciones" antes de "Resumen" (mismo tamaño de letra), y seis piezas nuevas — dos tarjetas de métrica clave, dos gráficos de barras horizontales uno al lado del otro con alto fijo de 5 filas y scroll, una tabla solo de números, un gráfico circular de tipo de donante y un gráfico de serie temporal con dos series de barras — todo pensado para que en PC no quede un gráfico alto al lado de un indicador pequeño, y en móvil se junten dos gráficos por fila cuando tenga sentido.

**Paleta nueva, validada antes de escribirse.** "Tipo de donante" es la primera serie categórica de identidad (4 tipos fijos) que tiene el panel — todo lo demás hasta hoy era magnitud (un solo matiz). Se corrió `scripts/validate_palette.js` de la skill de dataviz contra el fondo real de la app (no el de referencia): la primera combinación probada (verde/aqua/naranja/amarillo institucionales) falló banda de luminosidad y contraste; la segunda (verde/aqua/naranja/rojo) falló separación CVD porque naranja y rojo quedan indistinguibles entre sí. Se resolvió con el primer cuadrante de la paleta categórica de referencia de la skill (azul/naranja/aqua/amarillo) — pasa los seis chequeos en claro y oscuro, incluida la vuelta del anillo (amarillo↔azul, el par que cierra el círculo en un donut) — nuevos tokens `--viz-donor-individual/-empresa/-institucion/-anonimo` en los tres bloques sincronizados de `globals.css`. Institución y Anónimo quedan sub-3:1 de contraste contra la superficie clara — por eso el donut lleva siempre leyenda con cifra y porcentaje, nunca solo el color (la misma regla que la skill vuelve obligatoria a partir de 4 series).

**`src/components/panel/charts.tsx` gana tres piezas y una se generaliza:**
- **`DonutChart`** — anillo por trazo SVG (`stroke-dasharray`/`-dashoffset`, la técnica estándar para no calcular arcos a mano), con hueco de 3px entre segmentos igual que ya separa `DistributionBar` sus tramos, total en el centro, y leyenda con cifra y porcentaje siempre visible debajo.
- **`NumberTable`** — tabla genérica con encabezado fijo (`sticky`) y alto máximo con scroll propio; las celdas de datos son solo números alineados a la derecha (`tabular-nums`), la primera columna es la única con texto (la etiqueta de fila).
- **`HorizontalBarChart`** gana `maxVisibleRows` — con él, la lista de filas queda envuelta en un contenedor de alto fijo (`maxVisibleRows × 42px`, la altura aproximada de una fila) con `overflow-y-auto`. Aplicado a `byGroup` **y** al nuevo `byCategory` con el mismo valor (5), para que ambos midan exactamente lo mismo estén como estén de llenos — ya no hace falta recortar a "top 7 + Otros" como antes: lo que no cabe en 5 filas se ve bajando con scroll, no se pierde.
- **`DailyBarChart`** se generaliza de una sola serie a `series: {label,color,values}[]` — mismo componente para "Donaciones" solo (si algún día vuelve a usarse así) y para el "Donaciones y despachos" de hoy (dos barras por día, una al lado de la otra). Con una serie sigue mostrando el número sobre cada barra, como siempre; con dos o más lo quita (14 días × 2 cifras por columna es ruido, no señal) y lo manda a un `title` nativo por barra más una leyenda fija debajo — la identidad de cada serie nunca depende solo del color.

**`/panel/page.tsx` reestructurado:**
- **"Acciones" pasa a ser lo primero**, con el mismo `text-2xl font-black tracking-tight` que ya tenía "Resumen" (que baja a `<h2>`, y cada tarjeta de abajo baja de `<h2>` a `<h3>` para mantener el anidado semántico correcto).
- **Dos tarjetas nuevas en la fila de KPI** ("Donaciones recibidas", "Despachos realizados") — histórico, no del período, para que se lean como logros acumulados y no compitan con las alertas del período. La cuadrícula pasa de 4 a 6 tarjetas (`grid-cols-2 sm:grid-cols-3 lg:grid-cols-6`) y `StatTile`/`Action` ganan `h-full` para que todas midan lo mismo dentro de su fila aunque el texto de ayuda varíe de largo.
- **Reordenado en parejas pensadas por altura, no solo por tema:** "Distribución del inventario" (compacto) ahora va con "Tipo de donante" (compacto) en vez de con "Productos por grupo"; "Productos por grupo" pasa a la pareja explícita que pidió Juan Manuel, junto a "Stock por categoría" (mismo alto fijo de 5 filas). "Donaciones y despachos por día" y "Detalle de inventario por ubicación" quedan a ancho completo, a propósito: 14 columnas de dos barras cada una, o una tabla con varias columnas numéricas, se vuelven ilegibles exprimidas a la mitad de un teléfono — la única excepción deliberada a "junta dos gráficos por fila en móvil".
- **Un solo query de `donations`, sin filtro de fecha**, alimenta tres piezas a la vez (el total histórico de la tarjeta, el desglose por tipo de donante, y el conteo por día de los últimos 14) — antes la donación se leía dos veces (dentro y fuera del período) para cosas relacionadas.
- **"Stock por categoría"** reutiliza exactamente el mismo criterio que ya tenía "Productos con existencia por grupo" desde el 18 de agosto: cuenta referencias de producto distintas con saldo, nunca la cantidad sumada (mezclaría kilos con litros con unidades) — mismo bucle de agregación, ahora también por `category_id`.
- **"Detalle de inventario por ubicación"** reutiliza el mismo patrón de "Por Ubicar" que ya usa `/panel/inventario`: los renglones sin `location_id` se agrupan en su propia fila con ese nombre, en vez de perderse o romper la tabla.
- **"Despachos realizados"** cuenta `deliveries`, no `dispatches` — un despacho solo tiene entrega confirmada (y por tanto una fila en esa colección) cuando de verdad se completó, vía la ruta propia `confirm-delivery` del backend; contar `dispatches` habría incluido los que todavía están en camino.
...
### 2026-08-28 — CARTO se retira: mapas migrados a OpenFreeMap (vectorial, sin API key)

Pedido explícito de Juan Manuel, con captura: los dos mapas de la app (impacto en el panel, selector de dirección en solicitudes/despachos) empezaron a mostrar "API KEY REQUIRED" sobre las teselas. Investigado antes de tocar código: CARTO no solo empezó a exigir key para `basemaps.cartocdn.com` — su propia documentación anuncia el **retiro** de ese servicio raster, incluso para quien sí tenga key ("raster basemaps require an API key, and are being retired"). Pedir una key habría sido un parche de corta vida, no una solución — se resolvió con `AskUserQuestion`, ofreciendo tres alternativas reales (CARTO con key gratuita, OSM estándar, OpenFreeMap). Elegida **OpenFreeMap**: gratis sin límite, sin cuenta, sin key, sin tarjeta — mismo criterio que ya alejó a AKOPIA de Mapbox hacia Nominatim el 19 de agosto.

**Diferencia real con CARTO, no solo de proveedor:** OpenFreeMap sirve teselas **vectoriales** (MapLibre GL), no ráster — un `<TileLayer>` de Leaflet no las entiende. Se integran al mismo `MapContainer` de Leaflet que ya existía, vía `@maplibre/maplibre-gl-leaflet` (nuevo, junto con `maplibre-gl`): un componente sin salida visual (`MaplibreBasemap`, mismo patrón que `ClickToMove`/`RecenterOnFocus` en `map-picker.tsx`) que agrega la capa imperativamente con `useMap()`, sin tocar nada del resto de Leaflet — arrastrar el pin, clic para mover, `CircleMarker`, todo sigue igual.

**El mapa de impacto (`help-map.tsx`) no tenía un endpoint "sin nombres" equivalente a `light_nolabels` de CARTO.** Con teselas vectoriales el control es más fino, no más pobre: `useLabellessMapStyle()` (`src/lib/openfreemap.ts`) descarga el estilo `positron` completo una sola vez (cacheado a nivel de módulo, para que un mapa que se desmonta y remonta no repita la petición) y filtra las capas `type === "symbol"` — 19 de las 55 capas del estilo real, confirmado leyendo el JSON antes de dar el filtro por bueno — reproduciendo exactamente el mismo criterio ya documentado para CARTO: la diferencia entre un mapa de referencia y un mapa de vigilancia. El selector de dirección (`map-picker.tsx`) usa el estilo completo, con nombres — ahí sí hacen falta para confirmar el sitio exacto.

**CSP actualizada** (`src/proxy.ts`, la cabecera con nonce que reemplazó a la de nginx desde el 22 de agosto): se retiró `*.basemaps.cartocdn.com`/`*.openstreetmap.org` de `img-src` (ya no aplicaban — MapLibre pide estilo, teselas, sprites y glifos por `fetch`/XHR, no `<img>`, así que van en `connect-src`, no en `img-src`); se agregó `https://tiles.openfreemap.org` a `connect-src`; se agregó un `worker-src 'self' blob:` nuevo — MapLibre instancia su worker interno con `new Worker(url, {type:"module"})` derivado de `import.meta.url`, que Next.js resuelve a un chunk propio (mismo origen), con `blob:` solo como red de seguridad si algún bundler futuro cambia esa resolución.

Verificado de punta a punta contra el servidor real de la UNAL tras el despliegue: `npx tsc --noEmit`, `eslint` y `npm run build` (30 rutas) limpios, tanto en local como en el propio servidor; portada/`/panel`/`/api/health` responden 200 con la CSP nueva ya activa (confirmada leyendo la cabecera real de la respuesta); `tiles.openfreemap.org/styles/positron` responde 200 y trae la estructura esperada (55 capas, fuente vectorial `openmaptiles` + una fuente ráster de relieve para baja resolución); una tesela vectorial real (`.pbf`) se descargó con éxito. **Sin navegador disponible en esta sesión** — no se vio el mapa renderizado con teselas reales en pantalla, ni en el selector de dirección ni en el mapa de impacto; queda pendiente que Juan Manuel lo confirme visualmente.

### 2026-09-14 — `/panel/reportes`: informes en PDF, primera dependencia nueva desde el arranque del proyecto

Pedido explícito de Juan Manuel: una sección de Reportes en "Administración" que genere PDF con el estilo visual de la app, secciones activables por checkbox, párrafos descriptivos de cantidades respaldados por tablas/gráficos compactos — **no** listados extensos del catálogo. Resuelto con `AskUserQuestion` antes de escribir código: **`@react-pdf/renderer`** como motor (rompe la regla no escrita de "sin librerías nuevas sin discutirlo" de la tabla de decisiones de arriba, pero es justo el tipo de pieza que esa regla contempla agregar tras acordarlo — genera un PDF real descargable con un clic, sin depender del diálogo de impresión del navegador ni instalar Chromium en el servidor de la UNAL, 3.8 GB de RAM), roles con acceso (**admin, coordinación, comunicaciones** — mismo criterio que Historial) y datos personales (**agregados por defecto, con casilla explícita** para incluir principales donantes/operadores — apagada, el PDF se puede compartir sin exponer a nadie).

**Arquitectura: todo en el navegador, nada en el servidor.** `src/lib/reports/fetch.ts` lee PocketBase con la sesión propia del operador (así cada rol solo ve lo que sus reglas de acceso ya permiten — `adjustments` sencillamente no se pide si el rol no es admin/coordinación), `src/lib/reports/build.ts` agrega todo en memoria a diez secciones narrativas, y `src/components/reports/render-report.tsx` arma el PDF con `pdf(...).toBlob()` — el archivo nunca pasa por el frontend de Next.js como servidor, solo por el navegador de quien lo pide. Se carga con `import()` dinámico desde la página, para que el motor (con su módulo WebAssembly de layout, `yoga-layout`) no se descargue hasta que alguien de verdad genere un reporte.

**El balance en kilos, la pieza más delicada:** el saldo actual de `inventory` es la fuente de verdad (igual que ya hace `/panel` desde el 25 de agosto); el saldo al INICIO del periodo se reconstruye restando el efecto de cada movimiento posterior a esa fecha — mismo principio que la migración `060` del backend usó para reconciliar el ruido de punto flotante. `src/lib/reports/movements.ts` clasifica los nueve tipos de movimiento del backend (espejo comentado de `MOVEMENT_EFFECTS` en `pb_hooks/utils/helpers.js`) según si cambian la existencia TOTAL o solo la mueven entre cubetas — reservas, liberaciones y traslados no alteran el total, así que no cuentan como entrada/salida del reporte. Un cambio en el backend a esos tipos de movimiento (poco probable, pero posible) rompería este cálculo en silencio si no se actualiza aquí también — igual advertencia que ya lleva `src/lib/roles.ts` sobre su espejo de la jerarquía del backend.

**Gráficos hechos a mano** con `<Svg>`/`<View>` de `@react-pdf/renderer` (barras, barra apilada, anillo, columnas por periodo), misma paleta `--viz-*` ya validada (traducida a hex fijo — el PDF no lee variables CSS) y mismas reglas de la skill de dataviz que ya sigue `panel/charts.tsx`: extremo redondeado con base cuadrada, hueco de color superficie entre segmentos, etiqueta directa solo en el pico de cada serie, leyenda siempre presente con dos o más series.

**Bug real de layout, encontrado renderizando PDFs de verdad, no leyendo el código:** el motor Yoga (el mismo de React Native) colapsa la altura de una fila si el `flex` va en el `<Text>` en vez de en el `<View>` que lo envuelve — una tabla con varias columnas se veía comprimida a una línea sin texto visible. Corregido envolviendo cada celda en su propio `<View style={{flex}}>` (`DataTable` en `pdf-primitives.tsx`). Mismo patrón para cualquier fila con `flex` dentro de `<Columns>`: `<Block>` siempre asume que vive dentro de un contenedor `flexDirection: row`.

**Verificación real, sin Playwright** (esta sesión no lo tenía disponible): se transpiló el código TypeScript/JSX con la propia copia de `typescript` del proyecto (sin instalar nada nuevo) y se corrió `@react-pdf/renderer` directo en Node contra tres escenarios — un dataset sintético de un mes completo, generado aparte para no tocar `pb_data` real pero internamente consistente (los movimientos sintéticos sí cuadran con el inventario resultante); los datos reales de esta instancia local con "todo el histórico"; y "últimos 7 días" (un periodo sin ningún dato, el caso vacío real). Los tres PDF se revisaron página por página como imágenes — encontró y corrigió frases con gramática rota cuando una sección no tiene datos ("Se recibieron 0 donaciones..." pasó a "No se registraron donaciones..."), el bug de `flex` de arriba, y ajustes de tamaño (cabezote más alto, menos columnas por fila en "lo más pedido") para que ninguna tabla se viera apretada.

**Fuentes y escudo, mismo patrón que el resto de la identidad institucional pero para un motor distinto:** `@react-pdf` no puede usar `next/font` ni los `.woff2` que sirve el resto de la app — necesita `.ttf` reales vía `Font.register()`. Se copiaron cuatro pesos de Ancízar Sans desde `Plantilla-Pagina-Web_17-07-2026/fonts/` (ya en el repositorio raíz, nunca antes usados para esto) a `public/reports/fonts/`; el escudo (SVG con relleno blanco, pensado para el fondo oscuro del cabezote en pantalla) se rasterizó una vez a PNG con `sharp` — el PDF no soporta el `filter:invert()` que usa `unal-header.tsx`.

**CSP** (`src/proxy.ts`): se agregó `'wasm-unsafe-eval'` a `script-src`, exclusivo para que el navegador compile el módulo WebAssembly de `yoga-layout` — permite WASM, no `eval()` de JavaScript.

Verificado de punta a punta: `npx tsc --noEmit`, `eslint` y `npm run build` limpios (32 rutas). Desplegado en el servidor de la UNAL siguiendo el procedimiento estándar — la VPN de FortiClient se cayó justo después de terminar el build remoto (mismo síntoma intermitente ya documentado varias veces en este archivo), se reconectó y se completó el `systemctl restart`. Verificado con `curl --resolve` contra el dominio real: portada, `/api/health` y `/panel/reportes` responden 200.
