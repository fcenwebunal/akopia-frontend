...
export function GoogleButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className="flex w-full items-center justify-center gap-3 rounded border border-(--rule) bg-(--surface) px-6 py-3 font-bold hover:bg-(--surface-2) disabled:opacity-60"
    >
      {loading ? (
        <Spinner />
      ) : (
