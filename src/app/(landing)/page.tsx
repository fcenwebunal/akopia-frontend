...
export default async function HomePage() {
  ...
  return (
    <div className="font-sans">
      {/*
        Sin JavaScript, el IntersectionObserver de <ScrollReveal> nunca
        dispara — sin esto, las tarjetas animadas se quedarían con
        opacity:0 para siempre. Fuerza su estado final visible, sin
        animación, para quien navega sin JS.
      */}
      <noscript>
        <style>{`.akopia-reveal-hidden{opacity:1 !important;transform:none !important}`}</style>
      </noscript>
        ...
        <div className="relative z-[1] mx-auto max-w-5xl px-6">
          ...
          <div className="mt-10 grid grid-cols-2 gap-5 lg:grid-cols-4">
            {RECEIVING.map((item, index) => (
              <ScrollReveal key={item.title} className="akopia-drop-in h-full" delayMs={index * 110}>
                <div className="flex h-full flex-col gap-3.5 rounded-[18px] border border-black/5 bg-white p-6 shadow-[0_8px_24px_rgba(90,70,140,0.08)] transition-transform duration-200 hover:-translate-y-1.5 hover:shadow-[0_16px_30px_rgba(90,70,140,0.14)]">
                  <Image
                    src={item.icon}
                    alt={item.alt}
                    width={92}
      ...
      <section className="relative overflow-hidden bg-[#fbfaf6] py-16 sm:py-20">
        ...
        <div className="relative z-[1] mx-auto grid max-w-5xl gap-6 px-6 md:grid-cols-2">
          <ScrollReveal className="akopia-slide-left h-full">
            <div className="relative h-full overflow-hidden rounded-[22px] bg-gradient-to-br from-[#3f7a3e] to-[#1f4d2c] p-8 text-white">
          ...
          <ScrollReveal className="akopia-slide-right h-full">
            <div className="relative h-full overflow-hidden rounded-[22px] bg-gradient-to-br from-[#6a5aa8] to-[#453a7a] p-8 text-white">
              <a
